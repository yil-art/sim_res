...
let l1Root = null;                 // baked L1 origin (scene.json l1_root); null => no from-L1 readout
...
function pick(ev) {
  ...
  for (const L of clickLayers) {              // priority order: reach_normal precedes reach_xyz
    ...
    if (bi != null) {
      ...
      if (l1Root) {
        const p = L.points[bi];
        const d = [p[0] - l1Root[0], p[1] - l1Root[1], p[2] - l1Root[2]];
        const D = Math.hypot(d[0], d[1], d[2]);
        msg += ` · from L1 [${d[0].toFixed(3)}, ${d[1].toFixed(3)}, ${d[2].toFixed(3)}] (|d|=${D.toFixed(3)})`;
      }
