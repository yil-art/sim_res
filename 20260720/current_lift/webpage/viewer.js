...
const clickLayers = [];            // {name, obj, poses, coms} clickable point layers (emission order = pick priority)
let refPoint = null;               // baked readout anchor (scene.json ref_point); null => no from-ref readout
let totalMass = null;              // baked robot total mass (scene.json total_mass), kg
let comMarker = null;              // lazily-created COM sphere, moved on every pick
...
function pick(ev) {
  ...
  for (const L of clickLayers) {              // priority order: reach_normal precedes reach_xyz
    ...
    if (bi != null) {
      ...
      let msg = `${L.name} · point ${bi}`;
      if (refPoint) {
        const p = L.points[bi];
        const d = [p[0] - refPoint[0], p[1] - refPoint[1], p[2] - refPoint[2]];
        const D = Math.hypot(d[0], d[1], d[2]);
        msg += ` · from ref [${d[0].toFixed(3)}, ${d[1].toFixed(3)}, ${d[2].toFixed(3)}] (|d|=${D.toFixed(3)})`;
      }
      const c = L.coms && L.coms[bi];
      if (c) {
        if (!comMarker) {
          comMarker = new THREE.Mesh(new THREE.SphereGeometry(0.02, 16, 12),
            new THREE.MeshStandardMaterial({ color: 0xffa500 }));   // COM_MARKER_COLOR (255,165,0)
          scene.add(comMarker);
        }
        comMarker.position.set(c[0], c[1], c[2]);
        msg += ` · com [${c[0].toFixed(3)}, ${c[1].toFixed(3)}, ${c[2].toFixed(3)}]`;
        if (totalMass != null) msg += ` · mass ${totalMass.toFixed(3)} kg`;
      }
...
function addCheckbox(target, name, color, count, visible, onToggle) {
...
async function main() {
  ...
  refPoint = scn.ref_point || null;
  totalMass = scn.total_mass ?? null;
  ...
  scene.add(grid);
  if (refPoint) {                    // readout-anchor marker; REF_MARKER_COLOR (255,80,80)
    const m = new THREE.Mesh(new THREE.SphereGeometry(0.015, 16, 12),
      new THREE.MeshStandardMaterial({ color: 0xff5050 }));
    m.position.set(refPoint[0], refPoint[1], refPoint[2]);
    scene.add(m);
  }
  ...
  const groupVis = new Map((scn.groups || []).map((g) => [g.label, g.visible]));
  const groups = new Map();                 // label -> {master, kids, inputs:[], objs:[]}

  function ensureGroup(label) {
    if (groups.has(label)) return groups.get(label);
    const box = document.createElement('div');
    const header = document.createElement('label');
    const on = groupVis.get(label) ?? true;
    header.innerHTML = `<input type="checkbox" ${on ? 'checked' : ''}> <b class="grp">▸ ${label}</b>`;
    const master = header.querySelector('input');
    const caret = header.querySelector('b');
    const kids = document.createElement('div');
    kids.style.marginLeft = '14px';
    kids.style.display = 'none';                         // groups start collapsed (caret ▸)
    const g = { master, kids, inputs: [], objs: [] };
    master.addEventListener('change', () => {           // AND-gate: recompute children, keep their state
      for (let i = 0; i < g.inputs.length; i++)
        g.objs[i].visible = master.checked && g.inputs[i].checked;
    });
    caret.addEventListener('click', (e) => {
      e.preventDefault();
      const collapsed = kids.style.display === 'none';  // expand ▸→▾ / collapse ▾→▸, caret in sync
      kids.style.display = collapsed ? '' : 'none';
      caret.textContent = `${collapsed ? '▾' : '▸'} ${label}`;
    });
  ...
  function register(L, obj, swatch, count) {
    const g = L.group != null ? ensureGroup(L.group) : null;
    const container = g ? g.kids : panel;
    const gOn = g ? g.master.checked : true;
    obj.visible = L.visible && gOn;                     // initial: group AND layer
    const input = addCheckbox(container, L.name, swatch, count, L.visible, (v) => {
      obj.visible = v && (g ? g.master.checked : true);
    });
    if (g) { g.inputs.push(input); g.objs.push(obj); }
  }

  for (const L of scn.layers) {
    if (L.kind === 'lines') {
      const obj = buildLines(L); scene.add(obj);
      register(L, obj, L.color, null);
    } else if (L.kind === 'mesh') {
      const obj = buildMesh(L); scene.add(obj);
      register(L, obj, L.color, null);
    } else {                                   // 'points'
      const { obj, N } = buildPoints(L, scn.point_size); scene.add(obj);
      if (L.clickable) clickLayers.push({ name: L.name, obj, poses: L.poses, points: L.points,
                                          coms: L.coms || null });
      const swatch = L.color || (L.colors && L.colors.length ? L.colors[0] : [180, 180, 180]);
      register(L, obj, swatch, N);
    }
  }
  animate();
}
